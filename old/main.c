...
static void make_fbs() {
    ...
    for (int i = 0; i < (int) ARRLEN(images) /* NOLINT */; i++) {
        if (images[i]->id) {
...
static void init() {
    ...
    sg_setup(&(sg_desc) {
        .logger = (sg_logger) { .func = sg_logger_func }
    });
...
static int resolve_bullet(
    ...
    if (hit->type & T_OBJECT) {
        ...
        for (int i = 0, n = rand_n(&state->rand, 20, 50); i < n; i++) {
            particle_t *p = particle_new(level, hit->object.ptr->pos);
            if (!p) { continue; }

            p->type = PARTICLE_TYPE_FLARE;
            p->z = hit->object.ptr->z + 0.5f;
            p->vel_xyz =
                glms_vec3_scale(
                    rand_v3_hemisphere(&state->rand, VEC3(0, 0, 1)),
                    rand_f32(&state->rand, 8.0f, 16.0f));
            p->ticks = TICKS_PER_SECOND + rand_n(&state->rand, -15, 15);
            p->flare.color = VEC4(VEC3(0.0), 1.0f);
        }
        return PATH_TRACE_STOP;
    }

    // patricle spawn info
    vec3s part_pos, part_normal;
    ...
    if (hit->type & T_WALL) {
        ...
        part_normal = VEC3(side_normal(hit->wall.side), 0.0f);
        part_pos =
            glms_vec3_add(
                VEC3(
                    hit->swept_pos,
        ...
        part_pos =
            VEC3(
                ...
                hit->sector.ptr->planes[hit->sector.plane].z
                    + 0.001f * plane_sgn);
        part_normal = VEC3(0, 0, plane_sgn);
    }

    for (int i = 0, n = rand_n(&state->rand, 5, 10); i < n; i++) {
        particle_t *p = particle_new(level, glms_vec2(part_pos));
        if (!p) { continue; }
        ...
        p->vel_xyz =
            glms_vec3_scale(
                ...
                rand_f32(&state->rand, 1.0f, 10.0f));
        p->ticks = TICKS_PER_SECOND + rand_n(&state->rand, -15, 15);
        p->flare.color = VEC4(VEC3(0.5), 1.0f);
    }
...
static void do_bullet(object_t *player) {
    ...
    /* level_sectors_in_radius(state->level, player->pos, 4.0f, &near_sectors); */
...
static void do_ui() {
    ...
    ivec2s gun_pos =
        IVEC2(
            (TARGET_WIDTH - aabb_size(lookup.box_px).x) / 2 + 52,
            -20);
    ...
    const f32 mult = roundf(state->swing / (period / 2.0f)) * (period / 2.0f);
    if (fabsf(lv) < 1.0f) {
        state->swing =
            lerp(
                state->swing,
    ...
    /* state->swing = fmodf(state->swing, 1024.0f * period); */

    const bool is_aim =
        input_get(state->input, "mouse_right|left shift") & INPUT_DOWN;
    state->aim_time =
        max(state->aim_time + (is_aim ? state->time.dt : -state->time.dt), 0.0f);
    ...
    state->aim_factor =
        clamp(state->aim_factor + (state->time.dt * (is_aim ? 1 : -1)), 0.0f, 0.2f);

    const f32
        powx = lerp(1.0f, 0.15f, state->aim_factor / 0.2f),
        powy = lerp(1.0f, 0.8f, state->aim_factor / 0.2f);
    ...
    /* f32 cross = ease_in_out_circ(aimfactor / 0.2f); */

    /* if (aimfactor > 0.01f && aimtime > 2.0f) { */
    /*     cross -= ease_in_quint(clamp(aimtime - 1.5f, 0.0f, 1.0f)) * 0.3f; */
    /* } */

    /* if (aimfactor > 0.01f && aimtime > 3.5f) { */
    /*     cross -= min(aimtime - 3.0f, 3.0f) * 0.2f; */
    /* } */
    ...
    if (input_get(state->input, "mouse_left") & INPUT_PRESS) {
        ...
        particle_t *p = particle_new(state->level, glms_vec2(pos));
        if (p) {
            p->type = PARTICLE_TYPE_FLARE;
            p->z = pos.z;
            ...
            p->ticks = TICKS_PER_SECOND;
            p->flare.color = VEC4(VEC3(0.2), 1.0);
        }
...
static void frame() {
...
#if false
    igBegin("sound", NULL, 0);
    {
        igInputText(
            "TEXT", state->text, sizeof(state->text), 0, NULL, NULL);
        igInputFloat(
            "FREQ", &state->freq, 10.0f, 100.0f, "%.3f", 0);
        igInputFloat(
            "SPEED", &state->speed, 1, 2, "%.3f", 0);
        igInputFloat(
            "DECLINATION", &state->declination, 0.1f, 0.3f, "%.3f", 0);

        igCombo_Str(
            "WAVEFORM",
            &state->waveform,
            "SAW\0TRIANGLE\0SIN\0SQUARE\0PULSE\0NOISE\0WARBLE\0",
            8);
        ...
        if (igButton("PLAY", (ImVec2) {})) {
            if (!state->bus) {
                ...
                Soloud_play(state->sound->soloud, state->bus);
                state->filter0 = BiquadResonantFilter_create();
                Bus_setFilter(state->bus, 0, state->filter0);
            }
            ...
            if (!state->sp) {
                state->sp = Speech_create();
            }

            Speech_setText(state->sp, state->text);
            Speech_setParamsEx(
                state->sp,
                state->freq,
                state->speed,
                state->declination,
                state->waveform);
            Bus_play(state->bus, state->sp);
            /* Soloud_play(state->sound->soloud, state->sp); */
        }
    }
    igEnd();
#endif
    ...
    // update timers
    {
        ...
        state->time.frame_avg =
            (state->time.frame_avg * (59.0 / 60.0))
                + (state->time.delta * (1.0 / 60.0));
    ...
    if (state->mode == GAMEMODE_EDITOR
        ...
        const f32 speed =
            (input_get(state->input, "left ctrl") & INPUT_DOWN) ? 18.0f : 6.0f;
    ...
    if (state->allow_input) {
        ...
        if (input_get(state->input, "F4") & INPUT_PRESS) {
            palette_destroy(state->palette);
            palette_init(state->palette);
            palette_load_gpl(state->palette, "res/doompal.gpl");
        ...
        if (input_get(state->input, "F3") & INPUT_PRESS) {
            if (state->mode == GAMEMODE_EDITOR
                ...
                const int res =
                    editor_save_level(state->editor, state->editor->levelpath);

                if (res) {
                    WARN("error saving level to %s: %d",
    ...
    sg_push_debug_group("2D");
    {
        ...
        /* sgp_draw_filled_rect(10, 10, 10, 10); */
...
static void reload() {
    ...
    simgui_setup(&(simgui_desc_t) { 0 });
    igLoadIniSettingsFromDisk("imgui.ini");
    LOG("RELOAD!");
    ...
    level_dynlist_each(state->level->objects, it) {
        object_t *o = *it.el;
        o->type = &OBJECT_TYPES[o->type_index];
    }
}
